...
to setup
  ...
  create-turtles 100 [
    ...
    set color green
    set wealth max list 100 (random-normal 1000 200)
    set invested-in-tech? FALSE
    set sentiment 0
    set risk-tolerance wealth / 1000
  ]
  set tech-price 100
  set initial-price tech-price
  set crash-happened FALSE
  reset-ticks
end
...
to go
  ...
  update-tech-price
  ask turtles [ decide-investment ]
  ask turtles [ update-wealth ]
  tick
end

to update-tech-price
  if not crash-happened [
    let hype-factor count turtles with [invested-in-tech?] / count turtles
    set tech-price tech-price * (
      1
      + tech-growth-rate
      + (hype-factor * 0.03)
      + random-float tech-volatility - (tech-volatility / 2)
    )

    ; check for overvaluation compared to expected value
    let expected-price initial-price * ((1 + real-growth-rate) ^ ticks)
    if tech-price > crash-buffer * expected-price [
      set crash-happened TRUE
      set tech-growth-rate -0.1
    ]
  ]
  if crash-happened [
    set tech-price tech-price * (1 + tech-growth-rate + random-float 0.02 - 0.01)
  ]
end

to decide-investment
  let social-pressure mean [ sentiment ] of turtles in-radius 3
  let expected-return (tech-price - 100) / 100

  ; Combine expected return, peer pressure, scaled by risk
  set sentiment (w-price * expected-return + w-social * social-pressure) * risk-tolerance

  if sentiment > 0.1 [
    set invested-in-tech? TRUE
    set color red
  ]
  if sentiment < -0.1 [
    set invested-in-tech? FALSE
...
to update-wealth
  ifelse invested-in-tech? [
    set wealth wealth * (1 + ((tech-price - 100) / 100) / 10)
  ]
  [
    set wealth wealth * 1.001  ;; safe asset
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
...
78
Investors
count turtles with [invested-in-tech?]
17
1
11
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot tech-price"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot 100 * count turtles with [invested-in-tech?] / count turtles"
